...
def main(experiment_data: experiments_data_objects.ExperimentsInputData, debug: bool = False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(read_tasks) + 1) as executor:
    ...
    for k, v in write_data:
        experiment_results_file.write("{ip}:{timestamp}:{version}".format(ip=experiment_data.write_ip, timestamp=k, version=v) + "\n")
...
if __name__ == '__main__':
    main(experiments_data_objects.ExperimentsInputData(
        experiment_id=0,
